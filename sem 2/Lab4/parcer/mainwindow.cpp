...
{
    QString path = QFileDialog::getOpenFileName(this,"Open","D:","Cpp file (*.cpp)");
    QFile file(path);
...
void MainWindow::on_checkCode_clicked()
{
    code = ui->textEdit->toPlainText();
    if(!code.size()){
        QMessageBox::information(this,"Ошибка","Пустой текстовый блок");
...
int* MainWindow::getObjsAndArsData()
{
    ...
    for(size_t i = 0; i < size; ++i)
    {
        ...
        {
            temp.erase(first_bracket, last_bracket - first_bracket + 1);
        }
